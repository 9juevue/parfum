...
const brandList = ref([...brands.value]);
...
async function submit() {
    ...
    const result = isEditing.value
        ? brandUpdateSchema.safeParse(form)
        : brandStoreSchema.safeParse(form);
    ...
    isEditing.value
        ...
        : await storeBrand(result.data);
}

async function getBrand(brandId) {
    try {
        const response = await axios.get(route("admin.brands.show", brandId));
...
async function editBrand(item) {
    ...
    try {
        const brand = await getBrand(item.id);
        ...
        const copy = JSON.parse(JSON.stringify(brand));
...
async function storeBrand(brand) {
    try {
        const response = await axios.post(route("admin.brands.store"), brand);

        brandList.value.push(response.data);
...
async function updateBrand(brand) {
    try {
        ...
        if (response.data) {
            const idx = brandList.value.findIndex(
                (item) => item.id === response.data.id,
            );

            if (idx !== -1) {
                brandList.value[idx] = response.data;
            }
        }
...
async function deleteBrand(brand) {
    axios
        ...
        .then((res) => {
            let idx = brandList.value.findIndex((item) => item.id === brand.id);

            if (idx !== -1) {
                brandList.value.splice(idx, 1);
            }
                    ...
                    >
                        ...
                        <div class="mb-8 flex flex-col gap-1">
                            <label
                                ...
                                class="required-label font-semibold"
                                >Краткое описание бренда (для карточки
                                товара)</label
                            >
                            <div class="flex w-full flex-col">
                                <Skeleton
